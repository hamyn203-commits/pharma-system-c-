...
public class AuditService
{
    ...
    public AuditService(IDbContextFactory<Data.AppDbContext> contextFactory) => _contextFactory = contextFactory;

    public async Task<List<AuditLog>> GetAllAsync()
    {
        await using var db = await _contextFactory.CreateDbContextAsync();
        return await db.AuditLogs.OrderByDescending(a => a.CreatedAt).Take(500).ToListAsync();
    }

    public async Task LogAsync(string username, string action, string entity, string entityId, string details = "")
    {
        await using var db = await _contextFactory.CreateDbContextAsync();
        db.AuditLogs.Add(new AuditLog { Username = username, Action = action, Entity = entity, EntityId = entityId, Details = details, CreatedAt = DateTime.Now });
        await db.SaveChangesAsync();
    }
}
